...
if page == "📊 Giới thiệu & EDA":
    ...
    **Giá trị thực tiễn:** Giúp Quản trị viên (Admin) tiết kiệm 80% thời gian kiểm soát nội dung độc hại, duy trì môi trường mạng lành mạnh.
    ...
    # Hiển thị quy trình dưới dạng các bước
    steps = {
        "Bước 1: Clean Text": "Chuyển chữ thường, xóa URL, xóa khoảng trắng thừa.",
        "Bước 2: Teencode": "Chuyển các từ viết tắt (vcl, vl, ko,...) về từ gốc để máy hiểu.",
        "Bước 3: TF-IDF": "Biến đổi văn bản thành ma trận số dựa trên tần suất xuất hiện của từ.",
        "Bước 4: SVM": "Sử dụng thuật toán Support Vector Machine để tìm ranh giới phân loại 3 nhãn."
    }
    
    cols = st.columns(len(steps))
    for i, (step, desc) in enumerate(steps.items()):
        cols[i].inner_container().markdown(f"**{step}** \n*{desc}*")
