...
# -----------------------------
    ...
    # --- PHẦN 3: CHI TIẾT HIỆU NĂNG ---
    st.header("2. Phân tích chi tiết từng nhãn")

    col_res1, col_res2 = st.columns([1.2, 1])
    
    with col_res1:
        st.subheader("📑 Classification Report")
        # Chuyển đổi report sang DataFrame để hiển thị đẹp hơn
        report_df = pd.DataFrame(report_dict).transpose()
        # Loại bỏ các dòng trung bình để tập trung vào từng nhãn
        clean_report = report_df.iloc[:3, :].copy()
        clean_report.index = ["Bình thường 😐", "Tiêu cực 😡", "Tích cực 😍"]
        
        # Hiển thị bảng có màu sắc theo độ tốt của chỉ số
        st.dataframe(
            clean_report.style.background_gradient(cmap='YlGn', subset=['precision', 'recall', 'f1-score']),
            use_container_width=True
        )
        
        # Giải thích công thức ngắn gọn (Sử dụng Tooltip hoặc Expander nhỏ)
        with st.expander("📝 Giải thích chỉ số"):
            st.caption("**Precision:** Tỉ lệ dự đoán đúng trên tổng số ca dự đoán là nhãn đó.")
            st.caption("**Recall:** Tỉ lệ dự đoán đúng trên tổng số thực tế của nhãn đó.")
            st.caption("**F1-Score:** Trung bình điều hòa giữa Precision và Recall.")
    ...
    with col_res2:
        ...
        sns.heatmap(cm, annot=True, fmt='d', cmap='Blues', ax=ax_cm,
                    ...
                    cbar=False) # Ẩn thanh màu cho gọn
