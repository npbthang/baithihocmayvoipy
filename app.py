...
# -----------------------------
    ...
    with st.expander("🔍 Chi tiết thuật toán SVM & TF-IDF (Lý thuyết)", expanded=False):
        col_th1, col_th2 = st.columns(2)
        with col_th1:
            st.markdown("### 🔹 Thuật toán SVM")
            st.write("Tìm siêu phẳng tối ưu để phân tách dữ liệu:")
            st.latex(r"f(x) = \text{sign}(\mathbf{w}^T \mathbf{x} + b)")
        with col_th2:
            st.markdown("### 🔹 Kỹ thuật TF-IDF")
            st.write("Trọng số hóa đặc trưng văn bản:")
    ...
    # Hiển thị các chỉ số quan trọng nhất bằng Metric
    ...
    # --- PHẦN 3: CHI TIẾT HIỆU NĂNG & CÁC MA TRẬN MỞ RỘNG ---
    st.header("2. Phân tích chi tiết từng nhãn & Ma trận mở rộng")
    ...
    # --- PHẦN BỔ SUNG: BIỂU ĐỒ NÂNG CAO ---
    st.divider()
    st.subheader("📊 Biểu đồ phân tích chuyên sâu")
    
    c1, c2 = st.columns(2)
    
    with c1:
        st.markdown("**Đường cong Precision-Recall (PR Curve)**")
        # PR Curve cho bài toán đa lớp
        ...
        if hasattr(model, "predict_proba"):
            ...
            labels = ['Bình thường', 'Tiêu cực', 'Tích cực']
            
            for i in range(n_classes):
                precision, recall, _ = precision_recall_curve(Y[:, i], y_score[:, i])
                ax_pr.plot(recall, precision, color=colors[i], label=f'{labels[i]} (AUC={auc(recall, precision):.2f})')
            ...
            st.info("Mô hình không hỗ trợ predict_proba để vẽ PR Curve.")

    with c2:
        st.markdown("**Phân tích lỗi sai (Error Analysis)**")
        # Tìm những câu bị đoán sai để phân tích
        df_errors = df.copy()
        df_errors['predicted'] = y_pred
        errors = df_errors[df_errors['label'] != df_errors['predicted']].tail(5)
        
        if not errors.empty:
            # Map lại tên nhãn cho dễ đọc
            errors['label'] = errors['label'].map(label_map)
            errors['predicted'] = errors['predicted'].map(label_map)
            st.dataframe(errors[['comment', 'label', 'predicted']], use_container_width=True)
            st.caption("Các câu mô hình dự đoán nhầm thực tế.")
        else:
            st.success("Mô hình dự đoán đúng 100% trên tập dữ liệu này!")
